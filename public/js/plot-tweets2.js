  ...
  d3.selectAll(".tweet")
        ...
        .attr("r", function(d){ 
           ...
           // if (d.sentiment == search) {  
              if ((d.sentiment == search || d.tweet.search(search)!= -1) && Date.parse(d.date) >= firstdate && Date.parse(d.date) <= lastdate) { 
   ...
   copyData = copyData.filter(function(d){
        if(d.sentiment == search || d.tweet.search(search)!=-1){
            return true;
        }
...
//function to handle dateslider movement
function updateMap(firstdate,lastdate){
// function to resize circles
//var startdate = new Date(lastdata);
      //lastdate = Date.parse(data.values.max);
  //firstdate = Date.parse(data.values.min);  
    tweetstring = null;
    ...
    d3.selectAll(".tweet")
        ...
        .attr("r", function(d){ 
            ...
            //if keyword "default" was not found then perform this
              ...
              if ((d.sentiment == keyword || d.tweet.search(keyword)!= -1) && Date.parse(d.date) >= firstdate && Date.parse(d.date) <= lastdate) 
              {  
                 tweetstring = d.tweet.removeStopWords();//removes stopwords
                 //console.log(tweetstring);
                 tempData = tweetstring.split(" ");
  ...
  for (i in color.domain())
  {
    if(typeof emotionAndWords[color.domain()[i]][wordtag] == "undefined")
    {
      ...
      if(emotionAndWords[color.domain()[i]][wordtag] >= temp)
        {
      finalsentiment = color.domain()[i];
      temp = emotionAndWords[color.domain()[i]][wordtag];
        }
    }
  }
...
//function to fill up cloud with data
function updateWorldCloud(cities){
  d3.layout.cloud().size([600, 200])//width, height
        //.timeInterval(10)
        .words(cities.map(function(d) {
            return {text: d, tagColor: calculateFrequency(d) ,size: 10 + Math.random() * 90};
        }))
...
//function to build interactive stack bar chart (integrated with map)
...
//for gathering all the emotions
...
// load and display the World
    ...
    d3.json("./json/tweets.json", function(error, data) {
          g.selectAll("circle")
           ...
           .style("fill", function (d) {return color(cValue(d.sentiment));})
    ...
    //}
      ...
      //end of function that collects and draws the json map      
      });
  ...
  //end of main anAirport function
}
